...
Определяю полное имя домашнего каталога с помощью echo (рис. [-@fig:001]).

![Опредление каталога](image/1.png){#fig:001 width=70%}

вывод содержимого tmp с опциями -la, первая выводит полную информацию, вторая показывает скрытые файлы, проверка нал подкаталого - его нет, и домашний каталог со всеми разрешениями и владельцами файлов (рис. [-@fig:003]).

![вывод информации, проверка подкотолога](image/3.png){#fig:001 width=70%}

создает каталог newdir, создаем подкаталог morefun, создаем три каталога и удаляем их, пробуем удалить каталог - не получится тк он не пустой, удаляем подкаталог (рис. [-@fig:002]).

![Опредление каталога](image/2.png){#fig:001 width=70%}

используем каманду man для определения нужной опция - нам нужна -R (рис. [-@fig:004]).

![Опредление опции](image/4.png){#fig:001 width=70%}

используем каманду man для определения нужной опция - нам нужна -t (рис. [-@fig:005]).

![Опредление опции](image/5.png){#fig:001 width=70%}

используем каманду man для просмотра опций pwd cd rmdir mkdir rm - основные cd -PL (управление разрешение символических ссылок), pwd -P избежание ссылок, mkdir -p создание промежуточных каталогов, rmdir -p удаление каталога и родителей, rm -r рекурсивное удаление (рис. [-@fig:006]).

![Основные опции](image/6.png){#fig:001 width=70%}


с помощью history исполняем старые команды (рис. [-@fig:007]) (рис. [-@fig:008]).

![Первая команда](image/7.png){#fig:001 width=70%}

![Вторая команда](image/8.png){#fig:001 width=70%}
...
Я узнал несколько новых команд
